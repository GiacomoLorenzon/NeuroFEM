...
def helpcommand():
    ...
    print("\nData:\n")
    print(
        " -f (--solveproblem) \t<inputrfile> \tCommand to solver the problem with the data of this .prm file"
    )
    print(
        " -g (--generateparameterfile) \t<outputfile> \t Command to generate the .prm file for the problem"
    )
    print(
        " -c (--convergencetest) \t<convergenceiterations> \t Number of convergence iterations to perform"
    )
...
def main(argv, dir, dir_par):
    ...
    for opt, arg in opts:
        ...
        if opt in ("-g", "--generateparameterfile"):
            ...
            if conv > 0:
                ModelPRM.Createprm(filename, True)

            else:
                ModelPRM.Createprm(filename)
