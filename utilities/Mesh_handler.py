...
def MeshHandler(param, it):
    ...
    # No other choices are available, so print an ERROR MESSAGE
        ...
        print(
            "Type of Mesh choice not valid: please choose between File or Built-in options!"
        )
        sys.exit(0)
...
def BuiltInMeshGenerator(param, it):
    # Holed Cubic Case Mesh Generation
    if (
    ...
    elif param["Domain Definition"]["Built-in Mesh"]["Geometry Type"] == "Cube":
        ref = int(
            param["Domain Definition"]["Built-in Mesh"]["Mesh Refinement"]
        ) * (2**it)
        mesh = UnitCubeMesh(ref, ref, ref)

        return mesh

    # Sphere Case Mesh Generation
    elif (
        param["Domain Definition"]["Built-in Mesh"]["Geometry Type"] == "Sphere"
    ):
        # Geometrical Information about the Sphere
        r1 = param["Domain Definition"]["Built-in Mesh"]["Spherical Mesh"][
            "External Radius"
        ]
        r2 = param["Domain Definition"]["Built-in Mesh"]["Spherical Mesh"][
            "Internal Radius"
        ]
    ...
    # Square Case Mesh Generation
    elif (
        param["Domain Definition"]["Built-in Mesh"]["Geometry Type"] == "Square"
    ):
    ...
    # Square Case with crossed diagonals
    elif (
        param["Domain Definition"]["Built-in Mesh"]["Geometry Type"]
        == "Square crossed"
    ):
        ...
        mesh = UnitSquareMesh(ref, ref, "crossed")
    ...
    # Interval
        ...
        print(
            "Mesh Geometrical Type not valid! Control the Built-in meshes available types in the parameter file!"
        )
        sys.exit(0)
...
def GenerateCubicDomain(L1, L2):
    if L2 == False:
        ...
        BoxExt = mshr.Box(
            dolfin.Point(-L1 / 2, -L1 / 2, -L1 / 2),
            dolfin.Point(L1 / 2, L1 / 2, L1 / 2),
        )

        # Internal box generation
        BoxInt = mshr.Box(
            dolfin.Point(-L2 / 2, -L2 / 2, -L2 / 2),
            dolfin.Point(L2 / 2, L2 / 2, L2 / 2),
        )
