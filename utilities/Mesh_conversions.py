...
def vtutoxmlmesh_homog_skull_vent(param):
    ...
    XMLMeshFile = (
        param["InputFileName"][0 : len(param["InputFileName"]) - 4] + ".xml"
    )
    XMLMeshFileBDD = (
        param["InputFileName"][0 : len(param["InputFileName"]) - 4]
        + "_CellBDI.xml"
    )
    ...
    meshio.write_points_cells(
        ...
        cells=meshxml.cells,
        cell_data={"CellBDI": meshxml.cell_data["CellBDI"]},
    )
    print("\n")
...
def xdmftoh5_heterog(
    XDMFFileMesh, XDMFFileSubdomains, XDMFFileBoundaries, OutputFileName
):
    ...
    for face in dolfin.facets(mesh):
        ...
        app = middf.loc[
            (middf["0"] == face.midpoint()[0])
            & (middf["1"] == face.midpoint()[1])
            & (middf["2"] == face.midpoint()[2])
        ]
        ...
        if face.index() % 10000 == 0:
            advancement = (
                " - Status (last 10000 computed in "
                + str(math.trunc((time.time() - t) * 10000) / 10000)
                + " sec) : "
                + str(
                    math.trunc(face.index() / boundaries.size() * 10000) / 100
                )
                + " % (Remained: "
                + str(middf.shape[0])
                + ")"
