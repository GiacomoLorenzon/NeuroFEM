    ...
    if (
        ...
        L_el_DG_val = (
            ...
            - (
                2
                * G
                * inner(sym(grad(v)), (outer(uv, n) + outer(n, uv)) / 2)
                * ds_vent
            )
            - (l * div(v) * dot(uv, n) * ds_vent)
        )
    ...
    if (
        ...
        L_el_DG_val = (
            ...
            - (
                2
                * G
                * inner(sym(grad(v)), (outer(us, n) + outer(n, us)) / 2)
                * ds_skull
            )
            - (l * div(v) * dot(us, n) * ds_skull)
        )
...
def F_N_el(v, param, ds_vent, ds_skull, time=0, period=1):
...
def Fbel_DG_D(v, network, param, ds_vent, ds_skull, alpha, n, time=0, period=1):
    ...
    if (
        ...
        L_BCs = L_BCs - (alpha * inner(v, n) * p_vent * ds_vent)
    ...
    if (
        ...
        L_BCs = L_BCs - (alpha * inner(v, n) * p_skull * ds_skull)
...
def Fb_DG_D(
    ...
    if (
        ...
        L_BCs = L_BCs - (alpha * inner(uv, n) * q / dt * ds_vent)
    ...
    if (
        ...
        L_BCs = L_BCs - (alpha * inner(us, n) * q / dt * ds_skull)
...
def F_DG_D(
    ...
    if (
        ...
        if K == False:
            L_BCs = (
                L_BCs
                + (Kval * eta * deg * deg / h * p_vent * q * ds_vent)
                - (Kval * inner(grad(q), n) * p_vent * ds_vent)
            )
            ...
            L_BCs = (
                L_BCs
                + (Kval * eta * deg * deg / h * p_vent * q * ds_vent)
                - (Kval * inner(dot(K, grad(q)), n) * p_vent * ds_vent)
            )
    ...
    if (
        ...
        if K == False:
            L_BCs = (
                L_BCs
                + (Kval * eta * deg * deg / h * p_skull * q * ds_skull)
                - (Kval * inner(grad(q), n) * p_skull * ds_skull)
            )
            ...
            L_BCs = (
                L_BCs
                + (Kval * eta * deg * deg / h * p_skull * q * ds_skull)
                - (Kval * inner(dot(K, grad(q)), n) * p_skull * ds_skull)
            )
